    ...
    public static void Run()
    {
        ...
        while (true)
        {
            ...
            if (key.Key == ConsoleKey.Enter)
            {
                ...
                _history.Add(new MEAI.ChatMessage(MEAI.ChatRole.User, text));
                _msgLines.Add("");
    ...
    private static async Task StreamResponse()
    {
        ...
        while (true)
        {
            ...
            if (fullText.Length > 0)
                _history.Add(new MEAI.ChatMessage(MEAI.ChatRole.Assistant, fullText.ToString()));
...
{
    public static async Task<MEAI.ChatResponse> PatchReasoning(IEnumerable<MEAI.ChatMessage> msgs, MEAI.ChatOptions? opts, IChatClient inner, CancellationToken ct)
    {
        ...
        foreach (var msg in resp.Messages)
        {
            ...
            var p = OpenAI.Chat.ChatMessage.CreateAssistantMessage(cc);
    ...
    public static async IAsyncEnumerable<MEAI.ChatResponseUpdate> PatchStreamingReasoning(IEnumerable<MEAI.ChatMessage> msgs, MEAI.ChatOptions? opts, IChatClient inner, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct)
    {
